...
for i in range(len(blas_routines)):
    blas_routines[i] = list(
        filter(None, [x.strip() for x in blas_routines[i].split(",")])
    )
...
TEMPLATE_TEST_CASE( \""""
    ...
    cteVal = 2  # Constant value for the tests. It can be changed if needed.
    ...
    SECTION ( "n <= 0" ) {
        if( std::is_signed<idx_t>::value )
            CHECK( """
            + f_name
            + """(-1, x, incx, y, incy ) == real_t(0) );
        CHECK( """
            + f_name
            + """( 0, x, incx, y, incy ) == real_t(0) );
    }""",
        )
        countCases += 2
    ...
    SECTION( "y does not need to be set if beta = 0" ) {
        ...
        if f_name == "gemv":
            ...
            buffer += (
                f_name + "( layout, uplo, 2, alpha, A, 2, x, incx, real_t(0), y, 1 );",
            )
        buffer += (
            """
        CHECK( (!isnan(y[0]) && !isnan(y[1])) ); // i.e., they are not NaN
    ...
    SECTION( "A and x do not need to be set if alpha = 0" ) {
        ...
        if f_name == "gemv":
            buffer += (
                f_name
                + "( layout, trans, 2, 2, real_t(0), A_, 2, x_, 1, beta, y, 1 );",
            )
        else:
            buffer += (
                f_name + "( layout, uplo, 2, real_t(0), A_, 2, x_, 1, beta, y, 1 );",
            )
        buffer += (
            """
        CHECK( (!isnan(y[0]) && !isnan(y[1])) ); // i.e., they are not NaN
    ...
    SECTION( "x does not need to be set if alpha = 0" ) {
        ...
        """
            + f_name
            + """( layout, uplo, 2, real_t(0), x_, 1, A, 2 );
        CHECK( (!isnan(A[0]) && !isnan(A[1]) && !isnan(A[2]) && !isnan(A[3])) ); // i.e., they are not NaN
        std::fill_n(A, """
            + sizeArray
            + """, 1);
    ...
    SECTION( "Diagonal of A is not referenced if diag = 'U'" ) {
        ...
        """
            + f_name
            + """( layout, uplo, trans, Diag('U'), 2, A_, 2, x, incx );
    ...
    SECTION( "C := beta C if m, n > 0 and k = 0" ) {
        ...
        if f_name == "gemm":
            buffer += (
                "gemm( layout, transA, transB, m, n, 0, alpha, A, lda, B, ldb, real_t(2), C, ldc )",
            )
        elif f_name == "syrk" or f_name == "herk":
            buffer += (
                f_name
                + "( layout, uplo, trans, n, 0, alpha, A, lda, real_t(2), C, ldc )",
            )
        elif f_name == "syr2k" or f_name == "her2k":
            buffer += (
                f_name
                + "( layout, uplo, trans, n, 0, alpha, A, lda, B, ldb, real_t(2), C, ldc )",
            )
        buffer += (
            """ );
        CHECK( C[0] == C11 );
    ...
    SECTION( "Invalid complex case" ) {
        ...
        if f_name == "syrk":
            buffer += (
                "syrk( layout, uplo, Op('C'), n, k, alpha, A, lda, beta, C, ldc )",
            )
        elif f_name == "syr2k":
            buffer += (
                "syr2k( layout, uplo, Op('C'), n, k, alpha, A, lda, B, ldb, beta, C, ldc )",
            )
        elif f_name == "herk":
            buffer += (
                "herk( layout, uplo, Op('T'), n, k, alpha, A, lda, beta, C, ldc )",
            )
        elif f_name == "her2k":
            buffer += (
                "her2k( layout, uplo, Op('T'), n, k, alpha, A, lda, B, ldb, beta, C, ldc )",
            )
        buffer += (
            """, "trans" );
    }}""",
    ...
    SECTION( "C does not need to be set if beta = 0" ) {
        ...
        if f_name == "gemm" or f_name == "hemm" or f_name == "symm":
            if f_name == "gemm":
                buffer += (
                    f_name
                    + "( layout, transA, transB, 2, 2, 2, alpha, A, 2, B, 2, real_t(0), C_, 2 );",
                )
            else:
                buffer += (
                    f_name
                    + "( layout, side, uplo, 2, 2, alpha, A, 2, B, 2, real_t(0), C_, 2 );",
                )
            buffer += (
                """
        CHECK( (!isnan(C_[0]) && !isnan(C_[1]) && !isnan(C_[2]) && !isnan(C_[3])) ); // i.e., they are not NaN
            ...
            if f_name == "syrk" or f_name == "herk":
                buffer += (
                    f_name
                    + "( layout, uplo, trans, 2, 2, alpha, A, 2, real_t(0), C_, 2 );",
                )
            else:
                buffer += (
                    f_name
                    + "( layout, uplo, trans, 2, 2, alpha, A, 2, B, 2, real_t(0), C_, 2 );",
                )
            buffer += (
                """
    ...
    SECTION( "A and B do not need to be set if alpha = 0" ) {
        ...
        if f_name == "gemm":
            buffer += (
                f_name
                + "( layout, transA, transB, 2, 2, 2, real_t(0), A_, 2, B_, 2, beta, C, 2 );",
            )
        elif f_name == "hemm" or f_name == "symm":
            buffer += (
                f_name
                + "( layout, side, uplo, 2, 2, real_t(0), A_, 2, B_, 2, beta, C, 2 );",
            )
        else:
            buffer += (
                f_name
                + "( layout, uplo, trans, 2, 2, real_t(0), A_, 2, B_, 2, beta, C, 2 );",
            )
        buffer += (
            """
        CHECK( (!isnan(C[0]) && !isnan(C[1]) && !isnan(C[2]) && !isnan(C[3])) ); // i.e., they are not NaN
    ...
    SECTION( "A does not need to be set if alpha = 0" ) {
        ...
        """
            + f_name
            + """( layout, uplo, trans, 2, 2, real_t(0), A_, 2, beta, C, 2 );
        CHECK( (!isnan(C[0]) && !isnan(C[1]) && !isnan(C[2]) && !isnan(C[3])) ); // i.e., they are not NaN
        std::fill_n(C, """
            + sizeArray
            + """, 1);
    ...
    SECTION( "A does not need to be set if alpha = 0" ) {
        ...
        """
            + f_name
            + """( layout, side, uplo, trans, diag, 2, 2, real_t(0), A_, 2, B, 2 );
