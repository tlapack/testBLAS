...
TEMPLATE_TEST_CASE( "gemm does not propagate NaNs in C if beta = 0", "[NaN]", TEST_TYPES ) {
    ...
    TestType const A[m*k] = {1};
    TestType const B[k*n] = {2};

    TestType C0[m*n] = {NAN};
    gemm( layout, transA, transB, m, n, k, alpha, A, lda, B, ldb, real_t(0), C0, ldc );
    ...
    TestType C1[m*n] = {NAN};
    gemm( layout, transA, transB, m, n, k, alpha, A, lda, B, ldb, TestType(0), C1, ldc );
    for (idx_t i = 0; i < m*n; ++i) {
        INFO( "beta = ("<< get_type_name<TestType>() << ") 0" );
        REQUIRE( !isnan(C1[i]) );
    }

    TestType C2[m*n] = {NAN};
    gemm( layout, transA, transB, m, n, k, alpha, A, lda, B, ldb, idx_t(0), C2, ldc );
    for (idx_t i = 0; i < m*n; ++i) {
        INFO( "beta = ("<< get_type_name<idx_t>() << ") 0" );
        REQUIRE( !isnan(C2[i]) );
    }
}
...
TEMPLATE_TEST_CASE( "gemm does not propagate NaNs in A or B if alpha = 0", "[NaN]", TEST_TYPES ) {
    ...
    // Se A and B with numbers such that A*B does not overflow
    TestType const A[m*k] = {NAN};
    TestType const B[k*n] = {NAN};

    TestType C0[m*n] = {1};
    gemm( layout, transA, transB, m, n, k, real_t(0), A, lda, B, ldb, beta, C0, ldc );
    for (idx_t i = 0; i < m*n; ++i) {
    ...
    TestType C1[m*n] = {1};
    gemm( layout, transA, transB, m, n, k, TestType(0), A, lda, B, ldb, beta, C1, ldc );
    for (idx_t i = 0; i < m*n; ++i) {
        INFO( "alpha = ("<< get_type_name<TestType>() << ") 0" );
        REQUIRE( !isnan(C1[i]) );
    }

    TestType C2[m*n] = {1};
    gemm( layout, transA, transB, m, n, k, idx_t(0), A, lda, B, ldb, beta, C2, ldc );
    for (idx_t i = 0; i < m*n; ++i) {
        INFO( "alpha = ("<< get_type_name<idx_t>() << ") 0" );
        REQUIRE( !isnan(C2[i]) );
    }
}
