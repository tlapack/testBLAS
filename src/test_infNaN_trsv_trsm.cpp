...
// Constants
const blas::size_t N        = 128; // Number of rows
const blas::size_t P        = 3;  // Number of columns
const blas::size_t max_int  = 10;  // Values in the system range from 1 to (max_int-1)
const blas::size_t max_exp  = (const blas::size_t) ceil( log2(max_int) );
...
TEMPLATE_TEST_CASE( "trsv propagates Infs and NaNs from the triangular matrix to the solution",
    ...
    // Arrays
    const std::vector<blas::size_t> n_vec = { 2, 3, 10, N };
    TestType T[N*N];
    TestType x[N];
    ...
    for (const auto& n : n_vec) {
        ...
        // Put some ints in x
        for (blas::size_t i = 0; i < (blas::size_t) floor(n*(1-sparsity)); ++i)
        ...
        for (blas::size_t i = 0; i < n; ++i) {
            if( T_nanRow[i] && ( !isnan(x[i]) && !isinf(x[i]) ) ) {
                ...
                lineT << T(i,i);
                for (blas::size_t col = i+1; col < n; ++col)
                    lineT << ", " << T(i,col);
                UNSCOPED_INFO( "T[" << i << "," << i << ":] = " << lineT.str() );

                std::stringstream lineX;
                lineX << x[i];
                for (blas::size_t row = i+1; row < n; ++row)
                    lineX << ", " << x[row];
...
TEMPLATE_TEST_CASE( "trsm propagates Infs and NaNs from the triangular matrix to the solution",
    ...
    // Arrays
    const std::vector<blas::size_t> n_vec = { 2, 3, 10, N };
    const std::vector<blas::size_t> p_vec = { 1, P };
    TestType T[N*N];
    TestType x[N*P];
    ...
    for (const auto& n : n_vec) {
        ...
        // Put some ints in x
        for (blas::size_t i = 0; i < (blas::size_t) floor((n*p)*(1-sparsity)); ++i)
